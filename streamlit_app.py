"""
Streamlit UI for Credit Risk Analysis Demo - Phase 1
Features:
...
def create_company_table(df):
    ...
    with col4:
        ...
        if st.button("🔮 Predict SIC for Visible", type="primary"):
            st.success("✅ SIC prediction started for visible companies!")
    ...
    if len(paginated_df) > 0:
        ...
        for idx, (_, row) in enumerate(paginated_df.iterrows()):
            with st.container():
                ...
                with cols[-2]:
                    if st.button("🔮", key=f"predict_{start_idx + idx}", help="Predict SIC Code"):
                        st.success(f"✅ SIC prediction started for {row[available_columns[0]]}")

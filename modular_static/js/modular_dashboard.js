...
class ModularDashboard {
    ...
    setupEventListeners() {
        ...
        $(document).on('click', '.btn-predict-sic', (e) => {
            ...
            console.log('🔍 Predict SIC clicked for:', companyName);
            this.predictSIC(companyName, registrationNumber, sicCode);
        });
    ...
    async loadCompaniesData() {
        try {
            ...
            const companiesData = await window.ModularCore.makeApiCall(`companies?${params.toString()}`);
    ...
    renderCompaniesTable() {
        ...
        this.currentData.forEach((company, index) => {
            ...
            const companyIndex = company._index !== undefined ? company._index : ((this.currentPage - 1) * this.perPage + index);
            
            console.log(`🔍 Company ${globalIndex}: "${company['Company Name']}" - Dataset Index: ${companyIndex}`);
            ...
            const oldAccuracyClass = this.getAccuracyBadgeClass(oldAccuracyValue);
            ...
            const newAccuracyClass = this.getAccuracyBadgeClass(newAccuracyValue);
            ...
            const row = `
                <tr class="fade-in-up">
                    ...
                    <td>
                        <strong>${this.escapeHtml(company['Company Name'] || 'N/A')}</strong>
                    </td>
                    <td>
                        <small class="text-muted">${this.escapeHtml(company['Registration number'] || 'N/A')}</small>
                    </td>
                    <td>
                        <span class="badge bg-light text-dark">
                            ${this.escapeHtml(company.Country || 'N/A')}
                        </span>
                    </td>
                    <td class="text-center">
                        <code>${this.escapeHtml(company['UK SIC 2007 Code'] || 'N/A')}</code>
                    </td>
                    <td>
                        <small>${this.escapeHtml(company['UK SIC 2007 Description'] || 'N/A')}</small>
                    </td>
                    <td class="text-end">
                        ${this.formatRevenue(company['Sales (USD)'])}
                    </td>
                    <td class="text-center">
                        <span class="badge ${oldAccuracyClass}">${oldAccuracy}</span>
                    </td>
                    <td class="text-center">
                        <span class="badge ${newAccuracyClass}">${newAccuracy}</span>
                    </td>
                    <td class="${hasNewSIC ? 'text-success fw-bold' : 'text-muted'}">
                        ${newSIC || 'Not updated'}
                    </td>
                    <td>
                    ...
                    <td>
                        <button class="btn btn-outline-success btn-sm btn-action btn-predict-sic" 
                                data-company-index="${companyIndex}"
                                data-company-name="${this.escapeHtml(company['Company Name'])}"
                                data-registration="${this.escapeHtml(company['Company Registration Number'] || '')}"
                                data-sic-code="${this.escapeHtml(company['SIC Code (SIC 2007)'] || '')}"
                                title="Predict SIC">
    ...
    attachTableHandlers() {
        ...
        $(document).off('click', '.btn-update-revenue').on('click', '.btn-update-revenue', (e) => {
            ...
            this.handleUpdateRevenue(companyIndex);
        });
    }

    handleUpdateRevenue(companyIndex) {
        console.log('Update revenue for company index:', companyIndex);
        // This will be implemented to match the original functionality
        window.ModularCore.showSuccessBanner('Revenue update feature coming soon!');
    }
    ...
    async showCompanyDetails(companyIndex) {
        try {
            ...
            // Store company index for SIC prediction
     ...
     */
    async predictSIC(companyName, registrationNumber = null, sicCode = null) {
        ...
        try {
            ...
            if (sicCode && sicCode.trim() !== '') {
                requestData.sic_code = sicCode.trim();
            }
            
            const response = await window.ModularCore.makeApiCall('predict-sic', {
                method: 'POST',
                body: JSON.stringify(requestData)
            });
            
            console.log('🚀 SIC Prediction API Response:', response);
            
            // Start SIC workflow visualization with real agent data
            this.startSICWorkflow(response);
            
        } catch (error) {
            console.error('❌ Failed to predict SIC:', error);
            
            // Still show the workflow even if API fails, but with demo data
            $('#sic-panel-tab').tab('show');
    ...
    generateAIReasoningExplanation(result) {
        ...
        // If we have the currentData and currentPredictionIndex, use that
        if (!companyName && this.currentData && this.currentPredictionIndex !== undefined) {
            const selectedCompany = this.currentData[this.currentPredictionIndex];
    ...
    displaySICResults(data) {
        ...
        // Add event listener for Update Results button with safe parameter handling
        resultsContainer.find('.btn-update-score').off('click').on('click', (e) => {
            ...
            console.log('🎯 Update Results clicked:', { predictedSIC, confidence, companyName });
            
            if (predictedSIC && confidence !== undefined && companyName) {
                this.updateScoreFromPredictionWithCompany(predictedSIC, confidence, companyName);
            } else {
                console.error('❌ Missing button parameters:', { predictedSIC, confidence, companyName });
                this.showErrorMessage('Update Results button is missing required parameters. Please run SIC prediction first.');
            }
        });
    }
    ...
    async updateScoreFromPredictionWithCompany(predictedSIC, confidencePercentage, companyName) {
        console.log('💾 Update Score from prediction with explicit company:', predictedSIC, 'confidence:', confidencePercentage, 'company:', companyName);
        
        try {
            // Use the explicitly passed company name
            if (!companyName || companyName.trim() === '') {
                console.error('❌ No company name provided:', companyName);
                this.showErrorMessage('No company selected for update. Please run SIC prediction first.');
                return;
            }
            ...
            // Show the update results loading modal
            ...
            if (response.success) {
                ...
                $('#sicResults').prepend(`
                    <div class="alert alert-success alert-dismissible fade show" role="alert">
                        <strong>Success!</strong> SIC updated to ${response.new_sic} for ${response.company_name} with ${response.new_accuracy.toFixed(1)}% accuracy.
                        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                    </div>
                `);
                
                // Keep the SIC prediction modal open by not interfering with it
            ...
            console.error('Error updating score from prediction with company:', error);

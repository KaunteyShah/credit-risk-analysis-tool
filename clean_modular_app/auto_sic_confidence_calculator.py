...
This script automatically calculates existing_sic_confidence for new companies
when they are imported into the system. It should be run after data import
to populate confidence scores in sic_prediction_history table.
...
sys.path.insert(0, str(Path(__file__).parent / 'app_modules'))

# Import fuzzywuzzy for confidence calculation
try:
...
class AutoSICConfidenceCalculator:
    ...
    def calculate_confidence_score(self, business_description: str, sic_code: str, sic_description: Optional[str] = None) -> float:
        ...
        return min(max(base_confidence, 10.0), 90.0)  # Keep between 10-90%

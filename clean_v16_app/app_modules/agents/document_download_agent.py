...
class DocumentDownloadAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.cache_directory = config.get("processing.document_cache_dir", "data/downloaded_documents/pdfs")
        
        # Create cache directory if it doesn't exist
        os.makedirs(self.cache_directory, exist_ok=True)
        
        # Also create processed documents directory
        self.processed_directory = "data/downloaded_documents/processed"
    ...
    def download_by_document_id(self, document_id: str, company_name: str = "Unknown") -> Optional[DownloadedDocument]:
        """
        Download document directly using document_id.
        
        Args:
            document_id: The direct document ID from Companies House
            company_name: Company name for logging purposes
            
        Returns:
            DownloadedDocument object if successful, None otherwise
        """
        try:
            self.log_activity(f"Downloading document by ID: {document_id} for {company_name}")
            
            # Check cache first
            cached_content = None
            if self.cache_enabled:
                cached_content = self._get_cached_document(document_id)
                if cached_content:
                    self.log_activity(f"Using cached document {document_id}")
                    
                    # Create document info with available data
                    document_info = DocumentInfo(
                        document_id=document_id,
                        document_type="accounts",
                        filing_date="",
                        period_end_on=None,
                        period_start_on=None,
                        description=f"Financial document for {company_name}",
                        category="accounts",
            ...
            # Download document content if not cached
            content = self._download_document_content(document_id)
            
            if content:
                # Create document info with available data
                document_info = DocumentInfo(
                    document_id=document_id,
                    document_type="accounts",  # Assumed type for revenue extraction
                    filing_date="",
                    period_end_on=None,
                    period_start_on=None,
                    description=f"Financial document for {company_name}",
                ...
                # Create downloaded document object
                ...
                self.log_activity(f"Successfully downloaded document {document_id} ({len(content)} bytes)")
                return downloaded_doc
            else:
                self.log_activity(f"Failed to download document {document_id}", "ERROR")
                return None
    ...
    def _download_document_content(self, document_id: str) -> Optional[bytes]:
        ...
        try:
            ...
            # Use document API endpoint
            url = f"https://document-api.company-information.service.gov.uk/document/{document_id}/content"
            
            # Set proper headers for document download (PDF content)
            headers = {
                'Accept': 'application/pdf',  # Request PDF content, not JSON
                'User-Agent': 'Credit Risk System/1.0'
            }
            
            # Make request with authentication and correct headers
            response = requests.get(
                url, 
                auth=(self.ch_client.api_key, ''),  # Use basic auth like data API
                headers=headers,
                stream=True,
                timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                self.log_activity(f"Response headers: {dict(response.headers)}", "DEBUG")
                if response.text:
                    self.log_activity(f"Response body: {response.text[:200]}", "DEBUG")
                return None

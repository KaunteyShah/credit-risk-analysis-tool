...
try:
    ...
    class MockNumpy:
        ...
        def isnan(x):
            try:
                return x != x  # NaN property: NaN != NaN
            except:
                return False
...
class FileCompanyRepository(CompanyRepositoryInterface):
        ...
        try:
            ...
            for idx, (original_idx, row) in enumerate(data_subset.iterrows()):
                ...
                record = {
                    ...
                    'company_name': str(row.get('Company Name', '')),  # Map CSV to database field name
                    'country': str(row.get('Country', '')),  # Map CSV to database field name  
                    'employees_total': float(row['Employees (Total)']) if pd.notna(row.get('Employees (Total)')) else None,
                    'sales_gbp': float(row['Sales (GBP)']) if pd.notna(row.get('Sales (GBP)')) else None,
                    'uk_sic_2007_code': str(row.get('UK SIC 2007 Code', '')),  # Map CSV to database field name
                    'confidence': float(row.get('Old_Accuracy', 0)) if pd.notna(row.get('Old_Accuracy')) else 0,  # Map to database field name
                    'new_confidence': float(row.get('New_Accuracy', 0)) if pd.notna(row.get('New_Accuracy')) else 0,  # Map to database field name
                    'predicted_sic_code': str(row.get('New_SIC', '')) if pd.notna(row.get('New_SIC')) else None  # Map to database field name
                }
    ...
    def update_company_revenue(self, registration: str, revenue: float) -> bool:
        ...
        revenue_columns = ['Sales (GBP)', 'Revenue', 'Turnover']
        updated = False
    ...
    def get_revenue_statistics(self) -> Dict[str, Any]:
        ...
        revenue_col = None
        for col in ['Sales (GBP)', 'Revenue', 'Turnover']:
            if col in self._company_data.columns:
                revenue_col = col
                break
    ...
    def get_company_by_name(self, company_name: str, 
        ...
        try:
            ...
            if not matches.empty:
                ...
                logger.info(f"Found company by name '{company_name}': {company.get('company_name')}")
                return company

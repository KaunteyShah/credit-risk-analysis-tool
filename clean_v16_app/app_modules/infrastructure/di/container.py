...
def _configure_container(container: DIContainer) -> None:
    ...
    from app_modules.repositories.implementations.file_based.sqlite_company_repository import SQLiteCompanyRepository
    ...
    if database_type == 'sqlite':
        # Use SQLite repository with proper database field names
        logger.info("SQLite database type configured")
        container.register_singleton('company_repository', lambda: SQLiteCompanyRepository())
    else:
        # Default to SQLite repository (removing CSV dependency)
        logger.info("Defaulting to SQLite database repository")
        container.register_singleton('company_repository', lambda: SQLiteCompanyRepository())

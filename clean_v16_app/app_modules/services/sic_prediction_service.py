...
class SICPredictionService:
    ...
    def predict_sic_for_company(self, company_index: int, use_real_agents: bool = False, 
        ...
        try:
            ...
            return self._predict_sic_for_company_data(company, use_real_agents, app)
    ...
    def predict_sic_for_company_by_unique_id(self, unique_id: str,
                                           use_real_agents: bool = False, 
                                           app=None) -> Optional[Dict[str, Any]]:
        """
        Predict SIC code for a company by unique_id using modular approach
        Primary method for unique_id-based filtering
        """
        try:
            # Validate input
            company = self.repository.get_company_by_unique_id(unique_id)
            if not company:
                return {'error': f'Company not found with unique_id: {unique_id}'}
            
            return self._predict_sic_for_company_data(company, use_real_agents, app)
            
        except Exception as e:
            logger.error(f"Error in predict_sic_for_company_by_unique_id: {e}")
            return {'error': f'SIC prediction failed: {str(e)}'}
    ...
    def _predict_sic_for_company_data(self, company: Dict[str, Any], use_real_agents: bool = False, 
        ...
        try:
            ...
            baseline_accuracy = float(company.get('confidence', 0.0))
            ...
            # 🔑 UNIQUE_ID UPDATE: Use unique_id instead of company_index for prediction updates
            company_unique_id = company.get('unique_id')
            if company_unique_id:
                if not self.repository.update_company_prediction(
                    company_unique_id, predicted_sic, confidence, boosted_accuracy
                ):
                    logger.warning(f"Failed to update prediction for company unique_id: {company_unique_id}")
            else:
                logger.warning(f"No unique_id found for company: {company_name} - skipping prediction update")
            ...
            return {
                'success': True,
                'company_name': company_name,
                'company_index': company_index,  # Include company_index for approval workflow

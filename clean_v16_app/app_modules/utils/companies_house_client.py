...
class CompaniesHouseClient:
    ...
    def get_company_profile(self, company_number: str) -> Optional[CompanyData]:
        ...
        try:
            ...
            # Use proper Companies House API headers for versioning
            headers = {
                'Accept': 'application/json',  # Let API return latest version
                'User-Agent': 'data-risk-modernisation'
            }
            ...
            if response.status_code == 200:
                ...
                if 'version=' in content_type:
                    logger.info(f"API response version: {content_type}")
                ...
                logger.error(f"API version not acceptable (406): {response.text}")
                return None
            elif response.status_code == 410:
                logger.error(f"API version expired (410): {response.text}")
                return None
            else:
...
# Global client instance - initialized on module import
...
def get_companies_house_client():
    ...
    if companies_house_client is None:
        try:
            ...
            logger.warning(f"Could not initialize Companies House client: {e}")
            companies_house_client = None
    return companies_house_client

# Initialize the global client instance on module import
try:
    companies_house_client = CompaniesHouseClient()
